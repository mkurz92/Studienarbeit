% Funktion - Checkboxen untere Ebene Callback
function Vis_Check_unten_Callback(src, ~, data)
    % Global Variablen
    global P_Anz_gui
    global splot_5_2_0 %#ok<NUSED>
    global ax5
    for k=1:P_Anz_gui
    ...
    if (get(src,'Value') == get(src,'Max'))
        eval(['set(splot_5_2_', int2str(data), ', ''Visible'', ''on'');']);
    elseif (get(src,'Value') == get(src,'Min'))
        eval(['set(splot_5_2_', int2str(data), ', ''Visible'', ''off'');']);
    else
    end
